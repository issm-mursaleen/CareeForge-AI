...
class InterviewGenerator:
    ...
    async def generate(self, role: str, resume_text: str, count: int = 5) -> list[InterviewQuestion]:
        prompt = (
            ...
            f"technical and behavioral. Return ONLY a JSON array: "
            f'[{{"question":"...","category":"technical|behavioral|system_design",'
            f'"expected_topics":["..."]}}]\n\n'
            f"Resume:\n{resume_text[:4000]}"
        )
        response_text = await self.groq.generate_content_async(prompt, system_prompt=_GEN_SYSTEM)
        try:
    ...
    async def score_answer(self, question: str, answer: str, role: str) -> InterviewScore:
        prompt = (
        ...
        try:
            ...
            return InterviewScore(score=5.0, feedback=raw[:200], strengths=[], improvements=[])
